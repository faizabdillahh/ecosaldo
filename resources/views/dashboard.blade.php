<x-app-layout>
    ...
    <div class="py-4 md:py-6 px-3 md:px-4">
        ...
        @else
            {{-- ========== NASABAH — Best Practice UI/UX ========== --}}

            {{-- Saldo Card --}}
            <div class="bg-finance text-white rounded-2xl p-5 sm:p-6 shadow-lg mb-5">
                <div class="flex items-center justify-between mb-1">
                    <p class="text-xs sm:text-sm font-medium opacity-80">Saldo Aktif</p>
                </div>
                <p class="text-3xl sm:text-4xl font-bold tracking-tight">Rp {{ number_format($balance) }}</p>
            </div>

            {{-- Grafik Mini + Transaksi Terbaru --}}
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                <div class="sm:col-span-2 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Setoran 3 Bulan Terakhir</h3>
                    <canvas id="chartMini" height="120"></canvas>
                </div>
                <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Terbaru</h3>
                    @if($aktivitasTerbaru->count())
                        <div class="space-y-3">
                            @foreach($aktivitasTerbaru->take(3) as $s)
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 bg-eco-50 rounded-full flex items-center justify-center shrink-0">
                                    <svg class="w-4 h-4 text-eco" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-gray-900 truncate">{{ $s->jenisSampah->nama }}</p>
                                    <p class="text-xs text-gray-400">{{ $s->berat_kg }}kg · {{ $s->created_at->diffForHumans() }}</p>
                                </div>
                            </div>
                            @endforeach
                        </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    const last3 = [now-2, now-1, now].map(m => m < 0 ? m + 12 : m);
                    new Chart(ctx, {
                        type: 'line',
                        data: {
                            labels: last3.map(i => bulanLabels[i]),
                            datasets: [{
                                label: 'Setoran',
                                data: last3.map(i => dataBulanan[i+1] || 0),
                                borderColor: '#059669',
